...
int exec_client_requests(int cli_socket) {
    ...
    while (1) {
        ...
        build_cmd_buff(io_buff, &cmd_list);
        ...
        int rc = rsh_execute_pipeline(pipe_fd[1], &cmd_list);
        close(pipe_fd[1]);
 ...
 *                  get this value. 
 */
int rsh_execute_pipeline(int output_fd, command_list_t *clist) {
    ...
    int exit_code = WEXITSTATUS(pids_st[clist->num - 1]);
    for (int i = 0; i < clist->num; i++) {
...
int build_cmd_list(char *cmd_line, command_list_t *clist) {
    char *input = strdup(cmd_line); // Copy to avoid modifying original
    if (!input) return ERR_MEMORY;
    ...
    while (token && clist->num < CMD_MAX) {
        ...
        // Parse individual command into cmd_buff_t
        cmd_buff_t *cmd = &clist->commands[clist->num];
        if (build_cmd_buff(trimmed, cmd) != OK) {
            free_cmd_list(clist);
            free(input);
            return ERR_CMD_ARGS_BAD;
        }
    ...
    free(input);
    return (token != NULL) ? ERR_TOO_MANY_COMMANDS : OK;
}
...
int build_cmd_buff(char *cmd_line, cmd_buff_t *cmd) {
    ...
    if (!cmd->_cmd_buffer) return ERR_MEMORY;
    ...
    char *saveptr;
    char *token = strtok_r(cmd->_cmd_buffer, " ", &saveptr);
    
    while (token && cmd->argc < CMD_ARGV_MAX - 1) {
        // Handle redirection
        if (strcmp(token, "<") == 0) {
            token = strtok_r(NULL, " ", &saveptr);
            if (!token) return ERR_CMD_ARGS_BAD;
